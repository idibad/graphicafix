...
<div class="page-header">
    ...
    <div class="header-actions">
        <a href="generate_invoice_pdf.php?id=<?= $id ?>" target="_blank" class="btn btn-secondary"><i class="fas fa-inbox"></i> View PDF</a>
        <a href="generate_invoice_pdf.php?id=<?= $id ?>&dl=1" class="btn btn-secondary">⬇️ Download PDF</a>
        <a href="edit_invoice.php?id=<?= $id ?>" class="btn btn-secondary"><i class="fas fa-pencil-alt"></i>️ Edit</a>
        <?php if (!in_array($inv['status'], ['paid','cancelled'])): ?>
        <a href="invoice_details.php?id=<?= $id ?>&action=mark_paid"
           class="btn btn-primary"
           onclick="return confirm('Mark invoice as paid?')"><i class="fas fa-check-circle"></i> Mark as Paid</a>
        <?php endif; ?>
        <button class="btn btn-danger" onclick="deleteInvoice(<?= $id ?>, '<?= htmlspecialchars($inv['invoice_number']) ?>')"><i class="fas fa-trash"></i>️ Delete</button>
    </div>
</div>
...
<div class="alert-overdue">
    <i class="fas fa-exclamation-triangle"></i>️ <strong>This invoice is <?= $daysOverdue ?> days overdue.</strong>
...
<div class="inv-detail-grid">
    ...
    <div class="inv-detail-main">
            ...
            <div class="inv-meta-cols">
                ...
                <div class="inv-meta-col">
                    ...
                    <?php if (!empty($inv['client_email'])): ?>
                        <div class="inv-client-detail"><i class="fas fa-envelope"></i>️ <?= htmlspecialchars($inv['client_email']) ?></div>
                    <?php endif; ?>
                    <?php if (!empty($inv['client_phone'])): ?>
                        <div class="inv-client-detail"><i class="fas fa-phone"></i> <?= htmlspecialchars($inv['client_phone']) ?></div>
                    <?php endif; ?>
                    <?php if (!empty($inv['client_address'])): ?>
                        <div class="inv-client-detail"><i class="fas fa-map-marker-alt"></i> <?= nl2br(htmlspecialchars($inv['client_address'])) ?></div>
                    <?php endif; ?>
                </div>
        ...
        <div class="card" style="padding:0;overflow:hidden;margin-bottom:20px;">
            <div class="card-header" style="padding:16px 20px;">
                <div class="card-title"><i class="fas fa-edit"></i> Line Items</div>
            </div>
    ...
    <div class="inv-detail-sidebar">
        ...
        <div class="card sidebar-widget" style="border-top:3px solid <?= invStatusColor($inv['status']) ?>;">
            <div class="card-title" style="margin-bottom:16px;"><i class="fas fa-dollar-sign"></i> Payment Summary</div>
        ...
        <div class="card sidebar-widget">
            ...
            <div class="quick-actions-list">
                ...
                <a href="edit_invoice.php?id=<?= $id ?>" class="quick-action-btn">
                    <span><i class="fas fa-pencil-alt"></i>️</span> Edit Invoice
                </a>
                ...
                <a href="invoice_details.php?id=<?= $id ?>&action=mark_paid"
                   ...
                   onclick="return confirm('Mark this invoice as paid?')">
                    <span><i class="fas fa-check-circle"></i></span> Mark as Paid
                </a>
                <?php endif; ?>
