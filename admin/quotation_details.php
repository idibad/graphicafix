...
<div class="page-header">
    ...
    <div class="header-actions">
        <a href="generate_quotation_pdf.php?id=<?= $id ?>"     target="_blank" class="btn btn-secondary"><i class="fas fa-inbox"></i> View PDF</a>
        <a href="generate_quotation_pdf.php?id=<?= $id ?>&dl=1" class="btn btn-secondary">⬇️ Download PDF</a>
        <a href="edit_quotation.php?id=<?= $id ?>"             class="btn btn-secondary"><i class="fas fa-pencil-alt"></i>️ Edit</a>
        <?php if (!$convertedInv): ?>
        <a href="create_invoice.php?from_quot=<?= $id ?>"      class="btn btn-primary"><i class="fas fa-file-invoice-dollar"></i> Convert to Invoice</a>
        <?php endif; ?>
        <button class="btn btn-danger" onclick="deleteQuot(<?= $id ?>,'<?= htmlspecialchars($q['quotation_number']) ?>')"><i class="fas fa-trash"></i>️ Delete</button>
    </div>
</div>
...
<div class="alert-success">
    <i class="fas fa-check-circle"></i> <strong>Quotation created successfully!</strong>
...
<div class="alert-success">
    <i class="fas fa-check-circle"></i> <strong>Accepted.</strong> Converted to invoice
...
<div class="alert-success">
    <i class="fas fa-check-circle"></i> <strong>This quotation has been accepted.</strong>
    <a href="create_invoice.php?from_quot=<?= $id ?>">Convert to Invoice →</a>
</div>
<?php elseif ($q['status'] === 'rejected'): ?>
<div class="alert-rejected">
    <i class="fas fa-times-circle"></i> <strong>This quotation was rejected by the client.</strong>
</div>
<?php elseif ($isExpired): ?>
...
<div class="alert-warn">
    <i class="fas fa-exclamation-triangle"></i>️ <strong>Expiring soon.</strong> This quotation is valid for <?= $daysLeft ?> more day<?= $daysLeft!==1?'s':'' ?>, until <?= date('M d, Y', strtotime($q['valid_until'])) ?>.
</div>
<?php endif; ?>
...
<div class="qd-grid">
    ...
    <div class="qd-main">
            ...
            <div class="qd-meta-cols">
                <div class="qd-meta-col">
                    ...
                    <?php if (!empty($q['client_email'])): ?>
                        <div class="qd-client-detail"><i class="fas fa-envelope"></i>️ <a href="mailto:<?= htmlspecialchars($q['client_email']) ?>"><?= htmlspecialchars($q['client_email']) ?></a></div>
                    <?php endif; ?>
                    <?php if (!empty($q['client_phone'])): ?>
                        <div class="qd-client-detail"><i class="fas fa-phone"></i> <?= htmlspecialchars($q['client_phone']) ?></div>
                    <?php endif; ?>
                    <?php if (!empty($q['client_address'])): ?>
                        <div class="qd-client-detail"><i class="fas fa-map-marker-alt"></i> <?= nl2br(htmlspecialchars($q['client_address'])) ?></div>
                    <?php endif; ?>
                </div>
        ...
        <div class="card" style="margin-bottom:20px;">
            <div class="qd-section-label" style="margin-bottom:10px;"><i class="fas fa-clipboard"></i> Scope & Description</div>
            <p style="font-size:14px;color:#444;line-height:1.75;margin:0;white-space:pre-line;"><?= $q['notes'] ?></p>
        </div>
        <?php endif; ?>
        ...
        <div class="card" style="padding:0;overflow:hidden;margin-bottom:20px;">
            <div style="padding:16px 20px;border-bottom:1px solid #f0f0f0;">
                <div class="card-title"><i class="fas fa-dollar-sign"></i> Pricing Breakdown</div>
            </div>
        ...
        <div class="card" style="margin-bottom:20px;">
            <div style="background:#f9fafb;border-left:3px solid #024442;border-radius:0 8px 8px 0;padding:14px 18px;">
                <div class="qd-section-label" style="margin-bottom:8px;"><i class="fas fa-file-alt"></i> Terms & Conditions</div>
        ...
        <div class="card">
            <div class="qd-section-label" style="margin-bottom:16px;"><i class="fas fa-signature"></i>️ Signature</div>
            <div class="sig-block">
                <div class="sig-col">
    ...
    <div class="qd-sidebar">
        ...
        <div class="card sidebar-widget" style="border-top:3px solid <?= quotStatusColor($q['status']) ?>;">
            <div class="card-title" style="margin-bottom:14px;"><i class="fas fa-dollar-sign"></i> Quotation Value</div>
            ...
            <a href="invoice_details.php?id=<?= $convertedInv['id'] ?>" class="btn btn-secondary" style="width:100%;margin-top:14px;justify-content:center;">
                <i class="fas fa-file-alt"></i> View Invoice <?= htmlspecialchars($convertedInv['invoice_number']) ?>
            </a>
            <?php endif; ?>
        </div>
        ...
        <div class="card sidebar-widget">
            <div class="card-title" style="margin-bottom:16px;"><i class="fas fa-calendar-alt"></i> Timeline</div>
            <div class="timeline">
            ...
            <div class="quick-list">
                ...
                <a href="quotation_details.php?id=<?= $id ?>&action=mark_sent"
                   class="quick-btn" onclick="return confirm('Mark as sent to client?')">
                    <span><i class="fas fa-upload"></i></span> Mark as Sent
                </a>
                <?php endif; ?>
                ...
                <a href="quotation_details.php?id=<?= $id ?>&action=mark_accepted"
                   class="quick-btn quick-btn-green" onclick="return confirm('Mark as accepted?')">
                    <span><i class="fas fa-check-circle"></i></span> Mark as Accepted
                </a>
                <a href="quotation_details.php?id=<?= $id ?>&action=mark_rejected"
                   class="quick-btn quick-btn-warn" onclick="return confirm('Mark as rejected?')">
                    <span><i class="fas fa-times-circle"></i></span> Mark as Rejected
                </a>
                <?php endif; ?>
                <button onclick="deleteQuot(<?= $id ?>,'<?= htmlspecialchars($q['quotation_number']) ?>')"
                        class="quick-btn quick-btn-red">
                    <span><i class="fas fa-trash"></i>️</span> Delete Quotation
                </button>
            </div>
        </div>
        ...
        <!-- Breakdown -->
        <div class="card sidebar-widget">
            <div class="card-title" style="margin-bottom:14px;"><i class="fas fa-calculator"></i> Breakdown</div>
