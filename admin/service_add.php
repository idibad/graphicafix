...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO services 
        (title, description, icon, icon_style, status, starting_price, discount_percent, created_at, updated_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
    ...
    if (!empty($_POST['packages']) && is_array($_POST['packages'])) {
        foreach ($_POST['packages'] as $pkg) {
            ...
            $pstmt = $conn->prepare("INSERT INTO service_packages 
                (service_id, name, description, price, is_featured, status, created_at, updated_at) 
                VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())");
            $pstmt->bind_param("issdis", $service_id, $pkg_name, $pkg_desc, $pkg_price, $pkg_is_featured, $pkg_status);
            $pstmt->execute();
            ...
            if (!empty($pkg['features']) && is_array($pkg['features'])) {
                foreach ($pkg['features'] as $feat) {
                    $feat = trim($feat);
                    if ($feat === '') continue;
                    $fstmt = $conn->prepare("INSERT INTO service_package_features 
                        (package_id, feature, created_at) VALUES (?, ?, NOW())");
                    $fstmt->bind_param("is", $package_id, $feat);
                    $fstmt->execute();
                }
            }
        }
...
<div class="height-100">
    <div class="page-header">
        ...
        <p>Fill in the details below to add a new service to your offerings</p>
    </div>
    ...
    <div class="mp-tabs-container">
        <div class="mp-tab-buttons">
            ...
            <button type="button" onclick="showTab('media')">
                <span class="mp-tab-icon"><i class="fas fa-image"></i>️</span> Icon
            </button>
            <button type="button" onclick="showTab('packages')">
                <span class="mp-tab-icon"><i class="fas fa-box"></i></span> Packages
            </button>
        </div>
    </div>
    ...
    <div class="form-container">
        <form method="POST" enctype="multipart/form-data" id="serviceForm">
            ...
            <div id="media" class="tab">
                ...
                <div class="form-group">
                    <label>Icon Style (Random Gradient)</label>
                    <input type="text" name="icon_style" id="iconStyle" readonly placeholder="Will be auto-filled">
                </div>

                <div class="form-group">
                    <label>Preview</label>
                    <div id="iconPreview" style="font-size:40px;width:60px;height:60px;display:flex;align-items:center;justify-content:center;border-radius:8px;"></div>
                </div>
            </div>
            ...
            <div id="packages" class="tab">
                ...
                <p style="color:#888;font-size:14px;margin-bottom:20px;">Add pricing packages for this service. Each package can have its own features list.</p>
            ...
            <div class="form-actions">
                ...
                <button type="button" class="btn btn-secondary" onclick="window.location='services.php'">
                    Cancel
                </button>
                ...
                <button type="submit" class="btn btn-primary" id="submitBtn" style="display:none;">
                    <span><i class="fas fa-check"></i></span> Add Service
                </button>
            </div>
...
<script>
/* ── Tab logic (unchanged from original) ─────────────────────────────────── */
...
iconDropdown.addEventListener('change', function () {
    ...
    iconPreview.innerHTML = `<i class="${selectedIcon}" style="background:${gradient};-webkit-background-clip:text;-webkit-text-fill-color:transparent;"></i>`;
});
...
function addPackage() {
    ...
    card.innerHTML = `
        <div class="pkg-card-header">
            <h4><i class="fas fa-box"></i> Package ${idx + 1}</h4>
            <button type="button" class="pkg-remove-btn" onclick="removePackage('pkg-${idx}')" title="Remove package"><i class="fas fa-times"></i></button>
        </div>
        ...
        <div class="pkg-grid" style="margin-top:12px;">
            ...
            <div class="form-group" style="margin-bottom:0;display:flex;align-items:flex-end;">
                <label class="pkg-featured-row" style="margin:0;cursor:pointer;">
                    ...
                    Mark as Featured <span class="featured-badge"><i class="fas fa-star"></i> Featured</span>
                </label>
            </div>
        </div>

        <span class="pkg-features-label"><i class="fas fa-check-circle"></i> Features</span>
...
function renumberPackages() {
    document.querySelectorAll('.pkg-card').forEach((card, i) => {
        ...
        if (heading) heading.textContent = `<i class="fas fa-box"></i> Package ${i + 1}`;
    });
}
...
function addFeature(pkgIdx) {
    ...
    row.innerHTML = `
        ...
        <button type="button" class="pkg-feature-remove" onclick="removeFeature('feat-${pkgIdx}-${fi}')" title="Remove"><i class="fas fa-times"></i></button>
    `;
