...
<div class="page-header">
    <div>
        <h1><i class="fas fa-pencil-alt"></i>️ Edit Quotation</h1>
        <p>Editing <strong><?php echo htmlspecialchars($q['quotation_number']); ?></strong><?php echo $q['client_name'] ? ' · ' . htmlspecialchars($q['client_name']) : ''; ?></p>
    </div>
    <div style="display:flex;gap:8px;">
        <a href="quotation_details.php?id=<?php echo $id; ?>" class="btn btn-secondary">← Cancel</a>
    </div>
</div>

<div class="mp-tabs-container">
    <div class="mp-tab-buttons">
        <button type="button" class="active" onclick="showTab('cli')"><span class="mp-tab-icon"><i class="fas fa-building"></i></span> Client</button>
        <button type="button"                onclick="showTab('pri')"><span class="mp-tab-icon"><i class="fas fa-dollar-sign"></i></span> Pricing</button>
        <button type="button"                onclick="showTab('nts')"><span class="mp-tab-icon"><i class="fas fa-edit"></i></span> Notes & Terms</button>
        <button type="button"                onclick="showTab('sig')"><span class="mp-tab-icon"><i class="fas fa-signature"></i>️</span> Signature</button>
    </div>
</div>
...
<div id="cli" class="tab active">
    ...
    <div class="form-group">
        ...
        <select onchange="autofill(this.value)">
            ...
            <option value="<?php echo $cid; ?>" <?php echo $q['client_id']==$cid ? 'selected':''; ?>>
                <?php echo htmlspecialchars($c['client_name']); ?><?php echo $c['company'] ? ' — '.htmlspecialchars($c['company']):''; ?></option>
            <?php endforeach; ?>
        </select>
        <input type="hidden" name="client_id" id="hidCid" value="<?php echo $q['client_id']; ?>">
    </div>

    <div class="form-grid">
        <div class="form-group">
            <label>Client Name <span class="required">*</span></label>
            <input type="text" name="client_name" id="fName" value="<?php echo htmlspecialchars($q['client_name']); ?>" required>
        </div>
        <div class="form-group">
            <label>Company</label>
            <input type="text" name="client_company" id="fCompany" value="<?php echo htmlspecialchars($q['client_company'] ?? ''); ?>">
        </div>
    </div>
    <div class="form-grid">
        <div class="form-group">
            <label>Email</label>
            <input type="email" name="client_email" id="fEmail" value="<?php echo htmlspecialchars($q['client_email'] ?? ''); ?>">
        </div>
        <div class="form-group">
            <label>Phone</label>
            <input type="text" name="client_phone" id="fPhone" value="<?php echo htmlspecialchars($q['client_phone'] ?? ''); ?>">
        </div>
    </div>
    <div class="form-group">
        <label>Address</label>
        <textarea name="client_address" id="fAddress" style="min-height:70px;"><?php echo htmlspecialchars($q['client_address'] ?? ''); ?></textarea>
    </div>
    ...
    <div class="form-grid">
        <div class="form-group">
            ...
            <input type="text" name="quotation_number" value="<?php echo htmlspecialchars($q['quotation_number']); ?>" required>
        </div>
        <div class="form-group">
            <label>Currency</label>
            <select name="currency">
                <?php foreach (['PKR'=>'PKR — Pakistani Rupee','USD'=>'USD — US Dollar','EUR'=>'EUR — Euro','GBP'=>'GBP — British Pound'] as $v=>$l): ?>
                <option value="<?php echo $v; ?>" <?php echo $q['currency']===$v?'selected':''; ?>><?php echo $l; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
    <div class="form-grid">
        <div class="form-group">
            ...
            <input type="date" name="issue_date" value="<?php echo $q['issue_date']; ?>" required>
        </div>
        <div class="form-group">
            <label>Valid Until <span class="required">*</span></label>
            <input type="date" name="valid_until" value="<?php echo $q['valid_until']; ?>" required>
        </div>
    </div>
    <div class="form-group">
        <label>Status</label>
        <select name="status">
            ...
            <option value="<?php echo $v; ?>" <?php echo $q['status']===$v?'selected':''; ?>><?php echo $l; ?></option>
            <?php endforeach; ?>
        </select>
    </div>
</div>
...
<div id="pri" class="tab">
    ...
    <div class="form-grid" style="margin-top:14px;">
        <div class="form-group">
            <label>Tax %</label>
            <input type="number" name="tax_percent" id="taxPct" value="<?php echo $q['tax_percent']; ?>" step="0.01" min="0" max="100" oninput="recalc()">
        </div>
        <div class="form-group">
            <label>Discount Amount</label>
            <input type="number" name="discount_amount" id="discAmt" value="<?php echo $q['discount_amount']; ?>" step="0.01" min="0" oninput="recalc()">
        </div>
    </div>
...
<div id="nts" class="tab">
    <div class="form-group">
        ...
        <div id="notesEditor" style="height: 200px; background: #fff; font-family: inherit; border-radius: 0 0 8px 8px;"><?php echo $q['notes']; ?></div>
        <input type="hidden" name="notes" id="notesInput">
    </div>
    <div class="form-group">
        <label>Terms & Conditions</label>
        <textarea name="terms" style="min-height:130px;"><?php echo htmlspecialchars($q['terms'] ?? ''); ?></textarea>
    </div>
</div>
...
<div id="sig" class="tab">
    ...
    <div class="sig-preview-box">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:20px;">
            <div>
                ...
                <div class="sig-pname"  id="pName"><?php echo htmlspecialchars($q['signature_name']  ?: 'Your Name'); ?></div>
                <div class="sig-ptitle" id="pTitle"><?php echo htmlspecialchars($q['signature_title'] ?: 'Your Title'); ?></div>
                <div class="sig-pdate"  id="pDate">Date: <?php echo !empty($q['signature_date']) ? date('M d, Y', strtotime($q['signature_date'])) : date('M d, Y'); ?></div>
    ...
    <div class="form-grid" style="margin-top:20px;">
        <div class="form-group">
            ...
            <input type="text" name="signature_name" value="<?php echo htmlspecialchars($q['signature_name'] ?? ''); ?>"
                   placeholder="e.g. Ahmad Hassan"
                   oninput="document.getElementById('pName').textContent=this.value||'Your Name'">
        </div>
        <div class="form-group">
            <label>Title / Designation</label>
            <input type="text" name="signature_title" value="<?php echo htmlspecialchars($q['signature_title'] ?? ''); ?>"
    ...
    <div class="form-group">
        ...
        <input type="date" name="signature_date" value="<?php echo $q['signature_date'] ?? date('Y-m-d'); ?>"
...
<div class="form-actions">
    ...
    <button type="button" class="btn btn-secondary" onclick="window.location='quotation_details.php?id=<?php echo $id; ?>'">Cancel</button>
    <button type="button" class="btn btn-primary"   id="nextBtn" onclick="nextTab()">Next →</button>
    <button type="submit" class="btn btn-primary"   id="subBtn"  style="display:none;"><i class="fas fa-check"></i> Save Changes</button>
</div>
</form>
</div>
...
/* Quill fixes to match layout */
.ql-toolbar { border-radius: 8px 8px 0 0; background: #f9fafb; border-color: #e5e7eb !important; }
...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof Quill !== 'undefined') {
        notesEditor = new Quill('#notesEditor', {
            modules: { toolbar: [[{header:[2,3,false]}],['bold','italic','underline'],[{list:'ordered'},{list:'bullet'}],['clean']] },
            theme: 'snow',
            placeholder: 'Project description and extra notes...'
        });
...
const cData=<?php echo json_encode($cData); ?>;
...
function addRow(desc='',qty=1,price=''){
    ...
    tr.innerHTML=`
        ...
        <td><button type="button" class="remove-row-btn" onclick="removeRow(${i})"><i class="fas fa-times"></i></button></td>
    `;
...
const existing=<?php echo json_encode($existingItems); ?>;
